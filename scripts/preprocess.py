...
def get_sampleset_train(qiedian, yongdian, ctpt, huhao_taiqu):
    ganjingWithxiansun = set(list(huhao_taiqu.keys())).intersection(set(list(yongdian.keys())))
    ...
    gap = days_num-1
    sample = []
    for item in list(qiedianWithxiansun.union(ganjingWithxiansun)):
            try:
                ttvalue = reduce(lambda x,y:x*y, ctpt[item])
            except KeyError:
                ttvalue = 1
            yongdian[item] = [ttvalue * _ for _ in yongdian[item]]
            if max(yongdian[item]) < thresh and any(yongdian[item]):
                sample.append(item)

    train_x = np.array([yongdian[item] if item in yongdian else qiedian[item] for item in sample])
    ...
    for i in range(len(train_x)):
        ...
        sample_set[sample[i]] = [1 if sample[i] in qiedian else 0] + tempX

    return sample_set



def get_qiedian(path,maxday): # 提取窃电的编号,存成字典形式：qiedian = {‘a':[],'b':{}}

    # data = path + '\\低压窃电'
    data = path + '\\data\\低压窃电20151201-2161231'
    filename = os.listdir(data)
    ...
    for filetemp in filename:
        ...
        for line in islice(file, 1, None):
            ...
            daytemp = maxday - datetime.strptime(line[0][:-1], '%Y-%m-%d')
            daytempNum = 90 - daytemp.days
            try:

                if int(daytempNum) < 0:
                    continue
                figures[customer][daytempNum] = float(line[2])
                figures[customer][days_num + daytempNum] = float(line[3])
                figures[customer][2 * days_num + daytempNum] = float(line[4])
                figures[customer][3 * days_num + daytempNum] = float(line[5])
                figures[customer][4 * days_num + daytempNum] = float(line[6])
...
# path = 'C:\\Users\\sgcc\\project\\project-huzhou'
# yongdian, ctpt,start_end = get_yongdian(path)
# get_qiedian(path,start_end[1])