...
def put_yongdian_2db(figures, sql): # 用户用电量入库
    try:
        ...
        for user_id in figures:
                ...
                diff_values = ','.join(map(str,[float('%.2f' % (float(figures[user_id][i+1])-float(figures[user_id][i]))) for i in range(89)]))
                param.append((user_id,elec_values))
                param2.append((user_id,diff_values))

        sql.additem('elecconsume(user_id,elec_value)',param, format='(%s,%s)')

        sql.additem('diffconsume(user_id,diff_value)',param2, format='(%s,%s)')
